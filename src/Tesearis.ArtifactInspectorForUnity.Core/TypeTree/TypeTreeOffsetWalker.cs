...
namespace Tesearis.ArtifactInspectorForUnity.Core.TypeTree
{
    ...
    internal static class TypeTreeOffsetWalker
    {
        ...
        internal static long ComputeSize(TypeTreeNode node, long offset, IRandomAccessByteSource byteSource)
        {
            if (node == null) throw new ArgumentNullException(nameof(node));
            if (node.IsArrayLike) return ComputeArraySize(node, offset, byteSource);
            if (node.TypeName == "string") return 4 + ReadInt32LittleEndian(byteSource, offset);
            ...
            {
                var childSize = ComputeSize(child, currentOffset, byteSource);
        ...
        private static long ComputeArraySize(TypeTreeNode arrayNode, long offset, IRandomAccessByteSource byteSource)
        {
            if (arrayNode.Children.Count < 2)
            {
                throw new ArtifactInspectorException(
                    "Array-like type tree node '" + arrayNode.Name + "' (" + arrayNode.TypeName + ") has " +
                    arrayNode.Children.Count + " children; expected 2 (size and element template).");
            }

            var elementTemplate = arrayNode.Children[1];
            var count = ReadInt32LittleEndian(byteSource, offset);
            if (count < 0) throw new ArtifactInspectorException("Negative array length at offset " + offset + ".");
            RequireCountFitsRemainingBytes(count, offset + 4, byteSource);

            var currentOffset = offset + 4;
            for (var i = 0; i < count; i++)
            {
                var elementSize = ComputeSize(elementTemplate, currentOffset, byteSource);
        ...
        internal static void RequireCountFitsRemainingBytes(long count, long dataStartOffset, IRandomAccessByteSource byteSource)
        {
            ...
            {
                throw new ArtifactInspectorException(
                    "Length prefix " + count + " at offset " + dataStartOffset + " exceeds the " + remaining +
                    " bytes remaining in the underlying data. The file is likely truncated or corrupt.");
            }
        }
    }
}